...
# APIRouter 설정
router = APIRouter(tags=["mortgage"], responses={404: {"description": "Not found"}})
...
def get_plaid_sandbox_data(user_id: str) -> dict:
    ...
    try:
        ...
        # 테스트용 더미 데이터 반환
        return {"credit_score": 700, "accounts": [], "income": 50000, "debt": 10000}
...
async def mortgage_analysis(
    ...
    try:
        ...
        DTI = (debt / income) * 100
        monthly_payment = (loan_amount * (mortgage_rate / 100 / 12)) / (
            1 - (1 + mortgage_rate / 100 / 12) ** (-360)
        )
        ...
        return {
            ...
            "approval_details": {
                ...
                "소득 대비 월상환액 충족": (
                    "✅" if conditions["income_sufficient"] else "❌"
                ),
            },
        }
    except Exception as e:
...
async def get_historical_rates():
    ...
    try:
        ...
        # 테스트용 더미 데이터 반환
        return [
            {"date": f"2024-{month:02d}-01", "rate": 3.5 + (month * 0.1)}
...
async def analyze_mortgage(request: MortgageAnalysisRequest):
    ...
    try:
        ...
        approval_details = {
            "Credit Score": (
                "✅ Sufficient" if request.credit_score >= 620 else "❌ Insufficient"
            ),
            "DTI Ratio": "✅ Acceptable" if dti_ratio <= 43 else "❌ Too High",
            "LTV Ratio": "✅ Within Limit" if ltv_ratio <= 80 else "❌ Too High",
            "Down Payment": (
                "✅ Sufficient"
                if (request.down_payment / request.home_value * 100) >= 20
                else "❌ Insufficient"
            ),
        }
        ...
        # 전체 승인 여부 결정
        is_approved = all(
            detail.startswith("✅") for detail in approval_details.values()
        )
...
async def get_mortgage():
    try:
        return {"message": "Mortgage API is working", "status": "success"}
    except Exception as e:
